...
public class EnemyPower : Singleton<EnemyPower>
{
    [SerializeField] AIMovementStats _aiMovementStats;
    ...
    private void Start()
    {
        ...
        _maxPower = _aiMovementStats.Power;
    }
    ...
    public void UpdatePowerUI()
    {
        ...
        if (EnemyGoalCounter.Instance.IsGoal)
        {
            _currentPower += 3;
        }
    ...
    {
        if (EnemyInputManager.PowerWasPressed && _isFull)
        {
            ResetPower();
        ...
        switch (_abilityState)
        {
            case AbilityState.Ready:
                if (_isReady)
                {
                    _abilitySO.ActivateAbillity(gameObject, _ballTransform, _ballRB, _powerShotForce, AINewMovement.Instance.IsFacingRight);
                    _powerEffect.SetActive(false);
