...
public class AINewMovement : Singleton<AINewMovement>
{
    #region Variables
    enum EnemyStates { Idle, Run, Airborne, Sprint, WallJump, WallSlide, Dash, Kick }
    EnemyStates _state;
    bool _stateComplete;
    private bool _animComplete;
    public bool AnimComplete { get => _animComplete; private set => _animComplete = value; }

    [Header("Reference")]
    [SerializeField] private AIMovementStats MoveStats;
    ...
    private KnockbackPhysic _knockbackPhysic;
    ...
    private void FixedUpdate()
    {
        ...
        if (_isGrounded)
        {
            if (_knockbackPhysic != null && _knockbackPhysic.InHitStun)
                return;
            Move(MoveStats.GroundAcceleration, MoveStats.GroundDeceleration, EnemyInputManager.Movement);
        }
        else
        {
            //wall jump
            if (_useWallJumpMoveStats)
            {
                if (_knockbackPhysic != null && _knockbackPhysic.InHitStun)
                    return;
                Move(MoveStats.WallJumpMoveAcceleration, MoveStats.WallJumpMoveDeceleration, EnemyInputManager.Movement);
            }
            else //airborne
            {
                if (_knockbackPhysic != null && _knockbackPhysic.InHitStun)
                    return;
                Move(MoveStats.AirAcceleration, MoveStats.AirDeceleration, EnemyInputManager.Movement);
            }
        }
    ...
    private void UpdateCharacter(int selectedOption)
    {
        ...
        MoveStats = character.AIMovementStats;
    }
    ...
    private void Move(float acceleration, float deceleration, Vector2 moveInput)
    {
        if (!_isDashing)
        {
            if (Mathf.Abs(moveInput.x) >= MoveStats.MoveThreshold)
            {
                ...
                if (EnemyInputManager.RunIsHeld)
                {
    ...
    private void JumpChecks()
    {
        ...
        if (EnemyInputManager.JumpWasPressed)
        {
        ...
        if (EnemyInputManager.JumpWasReleased)
        {
    ...
    private void WallJumpCheck()
    {
        ...
        if (EnemyInputManager.JumpWasReleased && !_isWallSliding && !_isTouchingWall && _isWallJumping)
        {
        ...
        if (EnemyInputManager.JumpWasPressed && _wallJumpPostBufferTimer > 0f)
        {
            InitiateWallJump();
        }
    }
    ...
    {
        if (EnemyInputManager.DashWasPressed)
        {
            //ground dash 
    ...
    {
        SoundEffectManager.Instance.PlaySoundEffect(_dashClip, transform, 1);
        
        _dashDirection = EnemyInputManager.Movement;
    ...
    private void UpdateState()
    {
        switch (_state)
        {
            ...
            case EnemyStates.Airborne:
                UpdateAirborne();
                break;
            case EnemyStates.Sprint:
                UpdateSprint();
                break;
            case EnemyStates.WallJump:
                UpdateWallJump();
                break;
            case EnemyStates.WallSlide:
                UpdateWallSlide();
                break;
    ...
    {
        if (EnemyInputManager.Movement.x != 0 || !_isGrounded) _stateComplete = true;
    }
    ...
    private void UpdateAirborne()
    {
        ...
        _animator.Play("Enemy_Jump", 0, time);
        _animator.speed = 0;
    ...
    private void UpdateWallJump()
    {
        ...
        _animator.Play("Enemy_Wall_Jump", 0, time);
        _animator.speed = 0;
        if (_isGrounded || _isTouchingWall || _isWallSliding) _stateComplete = true;
    }
    ...
    {
        if (AnimComplete) _stateComplete = true;
    }*/
    #endregion
    ...
    private void SelectState()
    {
        ...
        if (_isGrounded)
        {
            /*if (InputManager.AttackWasPressed)
            {
                ...
                _state = PlayerStates.Kick;
                StartKick();
            }*/
            if (EnemyInputManager.DashWasPressed && !_isDashing && _dashOnGroundTimer < 0) //if dash
            {
                _state = EnemyStates.Dash;
                StartDash();
            }
            if (EnemyInputManager.Movement.x == 0) //if idle
            {
                _state = EnemyStates.Idle;
                StartIdle();
            }
            else if (EnemyInputManager.RunIsHeld) //if sprint
            {
                _state = EnemyStates.Sprint;
                StartSprint();
            }
            else //if run
            {
        ...
        else if (EnemyInputManager.DashWasPressed && !_isDashing && _numberOfDashesUsed < MoveStats.NumberOfDashes) //if air dash
        {
            _state = EnemyStates.Dash;
            StartDash();
        }
        else if (EnemyInputManager.JumpWasReleased && !_isWallSliding && !_isTouchingWall && _isWallJumping) //if wall jump
        {
            _state = EnemyStates.WallJump;
            StartWallJump();
        }
        else if (_isTouchingWall && !_isDashing) //if wall slide
        {
            _state = EnemyStates.WallSlide;
            StartWallSlide();
        }
        else
        {
            _state = EnemyStates.Airborne;
            StartAirborne();
        }
    }
    ...
    {
        _animator.Play("Enemy_Jump");
    }

    private void StartSprint()
    {
        _animator.Play("Enemy_Sprint");
    }

    private void StartWallJump()
    {
        _animator.Play("Enemy_Wall_Jump");
    }

    private void StartWallSlide()
    {
        _animator.Play("Enemy_Wall_Slide");
    }

    private void StartDash()
    {
        _animator.Play("Enemy_Dash");
    }

    /*private void StartKick()
    {
        _animator.Play("Player_Kick_1");
    }
