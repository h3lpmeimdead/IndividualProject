...
public class PlayerAttack : Singleton<PlayerAttack>
{
    ...
    {
        _animator.speed = 1;
        if (InputManager.AttackWasPressed && _attackTimeCounter >= _attackCooldown)
        {
    ...
    public IEnumerator KickTheBall()
    {
            ...
            SoundEffectManager.Instance.PlaySoundEffect(_kickClip, transform, .5f);
            ...
            for (int i = 0; i < _hits.Length; i++)
            {
                ...
                if (kickable != null)
                {
                    knockable.ApplyKnockback(_kickForce, this.transform.position, PlayerMovement.Instance.IsFacingRight);
                    flash.Flash();
    ...
    public IEnumerator DamageWhileAttackIsActive()
    {
        ...
        while(ShouldDamage)
        {
            ...
            for(int i = 0; i < _hits.Length; i++)
            {
                ...
                KnockbackPhysic knockbackable = _hits[i].collider.gameObject.GetComponent<KnockbackPhysic>();
                ImpactFlash flash = _hits[i].collider.gameObject.GetComponent<ImpactFlash>();
                ...
                if (damageable != null && !damageable.HasTakenDamage)
                {
                    ...
                    knockbackable.ApplyKnockback(_damageAmount, this.transform.position, PlayerMovement.Instance.IsFacingRight);
                    flash.Flash();
                    StopFrame.Instance.Stop(0.15f);
                    CameraShake.Instance.GloabalCameraShake(_impulseSource);
